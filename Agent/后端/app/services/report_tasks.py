...
from app.models import AssessmentRecord, Report, ReportTask, Town


def _append_database_summary(paths: list[Path], records: list[AssessmentRecord]) -> None:
    ...
    for path in paths:
        ...
        for cell, text in zip(table.rows[0].cells, ["镇街", "已复核记录", "状态", "评分条目"]):
            cell.text = text
        for town, items in by_town.items():
            ...
            row[3].text = str(sum(len(item.raw_payload.get("entries", [])) for item in items))
        document.save(path)
...
def run_report_task(task_id: str) -> None:
    ...
    with SessionLocal() as session:
        ...
        try:
            ...
            _append_database_summary(output_paths, records)
